...
export class Lenguajes extends Component {
    constructor(props) {
        ...
        this.url = (process.env.REACT_APP_API_URL || 'http://localhost:8080/catalogo/api/') + 'lenguajes/v1';
    }
    ...
    delete(key) {
        if (!window.confirm(`¿Seguro que quieres eliminar el lenguaje con ID ${key}?`)) return;
        this.setState({ loading: true });
...
function LanguagesView({ elemento, onCancel, listFilms }) {
