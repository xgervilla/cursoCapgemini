...
class ActorResourceTest {
	...
	void testGetAllString() throws Exception {
		...
		//en vez de hacer el srv.getX simulamos una llamda get desde el exterior
		mockMvc.perform(get("/api/actores/v1").accept(MediaType.APPLICATION_JSON))
			.andExpectAll(
					status().isOk(), 
					content().contentType("application/json"),
					jsonPath("$.size()").value(3)	//el $ indica la raíz, en este caso un array
					);
		//esquema equivalente (no funcional) de cómo sería con XML en vez de JSON
